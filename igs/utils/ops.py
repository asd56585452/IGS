...
def points_projection(points: Float[Tensor, "B Np 3"],
    ...
    # print(visible_pixels.shape, points_to_visible_pixels.shape, points_to_visible_pixels)
    # unique_indices, counts = torch.unique(points_to_visible_pixels, return_counts=True)
...
def select_points_bbox(points, bbox):   
    # points[*B,N,D]
    # bbox[*B,2,D] 
    # 获取在边界框内的点的索引
    # print(points[:,:, 0].shape, bbox.shape, bbox[:,0,0].shape)
    if len(points.shape)==2:
        points = points.unsqueeze(0)
        bbox = bbox.unsqueeze(0)
    indices = torch.where(
        (points[:,:, 0] >= bbox[:,0,0,None]) & (points[:,:, 0] <= bbox[:,1,0,None]) &
        (points[:,:, 1] >= bbox[:,0,1,None]) & (points[:,:, 1] <= bbox[:,1,1,None]) &
        (points[:,:, 2] >= bbox[:,0,2,None]) & (points[:,:, 2] <= bbox[:,1,2,None])
    )
    return indices

def position_embedding(xyz, dim=48):
    # e = torch.pow(2, torch.arange(dim // 6)).float() * np.pi
    # e = torch.stack([
    #     torch.cat([e, torch.zeros(dim // 6),
    #                 torch.zeros(dim // 6)]),
    #     torch.cat([torch.zeros(dim // 6), e,
    #                 torch.zeros(dim // 6)]),
    #     torch.cat([torch.zeros(dim // 6),
    #                 torch.zeros(dim // 6), e]),
    # ]).to(xyz)
    e = torch.logspace(0, 1, steps=dim // 6, base=2).float() * np.pi
    e = torch.stack([
        torch.cat([e, torch.zeros(dim // 6), torch.zeros(dim // 6)]),
        torch.cat([torch.zeros(dim // 6), e, torch.zeros(dim // 6)]),
        torch.cat([torch.zeros(dim // 6), torch.zeros(dim // 6), e]),
    ]).to(xyz)
    projections = torch.einsum(
        '...d,de->...e', xyz, e)  # .permute(2, 0, 1)
    # print(projections.max(), projections.min())
    embeddings = torch.cat([projections.sin(), projections.cos()], dim=-1)
    if embeddings.shape[-1] < dim:
        # print(embeddings)
        pad_zeros = embeddings.new_zeros(*embeddings.shape[:-1], dim-embeddings.shape[-1])
        embeddings = torch.cat([embeddings, pad_zeros], dim=-1)
    # print(embeddings, embeddings.shape)
    return embeddings

def perspective_projection(points: Float[Tensor, "B Np 3"],
                    c2ws: Float[Tensor, "B 4 4"],
                    intrinsics: Float[Tensor, "B 3 3"],
                    local_features: Float[Tensor, "B C H W"]):
    #不考虑点云之间的遮挡，作为消融的函数
    w2c = torch.inverse(c2ws)
    # points_hom = torch.cat([points, torch.ones_like(x_cam)], dim=1).reshape(-1,4,H*W)
    R = w2c[:,:3,:3]
    T = w2c[:,:3,3]
    
    # 将点云从世界坐标系转换到相机坐标系
    points_cam = torch.matmul(R, points.permute(0, 2, 1)) + T.unsqueeze(2)  # (B, 3, Np)
    
    # 相机坐标系到图像坐标系的转换
    points_image = torch.matmul(intrinsics, points_cam)  # (B, 3, Np)
    points_image = points_image / points_image[:, 2, :].unsqueeze(1)  # 归一化到图像平面
    
    # 获取图像平面上的点 (B, 2, Np)
    points_image = points_image[:, :2, :]
    
    # 将点云投影到图像平面上的点归一化到 [-1, 1] 的范围
    B, _, H, W = local_features.shape
    points_image_normalized = points_image.clone()
    points_image_normalized[:, 0, :] = 2 * points_image_normalized[:, 0, :] / (W) - 1  # 归一化 u 坐标
    points_image_normalized[:, 1, :] = 2 * points_image_normalized[:, 1, :] / (H) - 1  # 归一化 v 坐标
    
    # 将投影点转换为网格格式
    grid = points_image_normalized.unsqueeze(1).permute(0,1,3,2)  # (B, 1, 2, Np)
    
    # 使用 F.grid_sample 获取特征
    features = F.grid_sample(local_features, grid, align_corners=False)  # (B, C, 1, Np)
    
    # 返回特征
    return features.squeeze(2).permute(0, 2, 1)  # (B, Np, C)
