...
from igs.utils.ops import get_activation
...
class CombineNet(BaseModule):
    @dataclass
    class Config(BaseModule.Config):
        in_channels_1: int = 128
        in_channels_2: int = 3
    ...
    def configure(self) -> None:
        ...
        self.conv_in_1 = nn.Sequential(
            nn.Conv2d(self.cfg.in_channels_1, self.cfg.in_channels_1, kernel_size=3, stride=1, padding=1),
            nn.InstanceNorm2d(self.cfg.in_channels_1),
            nn.ReLU()
        )
        self.conv_in_2 = nn.Sequential(
            nn.Conv2d(self.cfg.in_channels_2, self.cfg.in_channels_2, kernel_size=3, stride=1, padding=1),
            nn.InstanceNorm2d(self.cfg.in_channels_2),
            nn.ReLU()
        )
        self.conv_out = nn.Sequential(
            nn.Conv2d(self.cfg.in_channels_2 + self.cfg.in_channels_1, self.cfg.out_channels, kernel_size=3, stride=1, padding=1),
            # nn.InstanceNorm2d(self.cfg.out_channels),
            # nn.ReLU()
        )
        self.unet = igs.find(self.cfg.unet_cls)(self.cfg.unet)
    ...
    def forward(
        ...
        input1 = self.conv_in_1(input1)
        # ic(input1.max(), input1.min(), input1.mean(), input1.std())
        input2 = self.conv_in_2(input2)
        # ic(input2.max(), input2.min(), input2.mean(), input2.std())

        combine_input = self.conv_out(torch.cat([input1, input2], dim=1))
        out = self.unet(combine_input)
        return out