...
class FeatureTransformer(BaseModule):
    ...
    def configure(self) -> None:
        ...
        self.nhead = self.cfg.nhead

        self.layers = nn.ModuleList([
            TransformerBlock(d_model=self.cfg.d_model,
                             nhead=self.cfg.nhead,
                             ffn_dim_expansion=self.cfg.ffn_dim_expansion,
                             )
    ...
    def forward(self, feature0, feature1,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # update feature1
            concat1 = torch.cat(concat0.chunk(chunks=2, dim=0)[::-1], dim=0) #相当于把它反转了一下
            # print(len(concat0.chunk(chunks=2, dim=0)))
            # print(concat0.chunk(chunks=2, dim=0)[0].shape)
            # print(concat0.chunk(chunks=2, dim=0)[::-1])
            # print("concat0", concat0.shape, "concat1",concat1.shape)
            # exit()
...
class FeatureTransformerMy(BaseModule):
    @dataclass
    class Config(BaseModule.Config):
        num_layers:int =2
        # self_num_layers:int=2
        d_model:int =128
        nhead:int =1
        ffn_dim_expansion:int =4
    def configure(self) -> None:
