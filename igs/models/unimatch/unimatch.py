...
class UniMatch(BaseModule):
    ...
    class Config(BaseModule.Config):
        ...
        attn_type: Optional[str] = 'swin'
        attn_splits_list: Optional[list] = (2,)
        corr_radius_list: Optional[list] = (-1,)
        prop_radius_list: Optional[list] = (-1,)
        num_reg_refine: int = 1
        pred_bidir_flow: Optional[bool] = False
        reg_refine: bool = False

        pretrained_model_name_or_path: str = ""

        as_backbone: bool = True
    cfg = Config

    def configure(self) -> None:
        super().configure()
        self.feature_channels = self.cfg.feature_channels
        self.num_scales = self.cfg.num_scales
        self.upsample_factor = self.cfg.upsample_factor
        # CNN
        self.backbone = CNNEncoder(output_dim=self.cfg.feature_channels, num_output_scales=self.cfg.num_scales)

        # Transformer
        self.transformer = igs.find(self.cfg.transformer_cls)(self.cfg.transformer)
        self.reg_refine = self.cfg.reg_refine
        if not self.cfg.as_backbone:
            self.feature_flow_attn = SelfAttnPropagation(in_channels=self.cfg.feature_channels)

               
            if not self.cfg.reg_refine :
            # convex upsampling simiar to RAFT
            # concat feature0 and low res flow as input
                self.upsampler = nn.Sequential(nn.Conv2d(2 + self.cfg.feature_channels, 256, 3, 1, 1),
                                            nn.ReLU(inplace=True),
                                            nn.Conv2d(256, self.upsample_factor ** 2 * 9, 1, 1, 0))
        if self.cfg.pretrained_model_name_or_path:

                    ckpt = torch.load(self.cfg.pretrained_model_name_or_path)
                    strict_mode = False if self.cfg.as_backbone else True
                    self.load_state_dict(ckpt['model'],strict = strict_mode)
    ...
    def forward(self, img0, img1,
        ...
        attn_splits_list = self.cfg.attn_splits_list
        corr_radius_list = self.cfg.corr_radius_list
        prop_radius_list = self.cfg.prop_radius_list
        num_reg_refine = self.cfg.num_reg_refine
        pred_bidir_flow = self.cfg.pred_bidir_flow
        if pred_bidir_flow:
            assert task == 'flow'
        ...
        for scale_idx in range(self.num_scales):
            ...
            if self.training:
                flow_bilinear = self.upsample_flow(flow, None, bilinear=True, upsample_factor=upsample_factor,
                                                    is_depth=task == 'depth')
                flow_preds.append(flow_bilinear)#1
    ...
    #         # add position to features
    #         feature0, feature1 = feature_add_position(feature0, feature1, 2, self.cfg.feature_channels)
