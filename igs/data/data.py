...
class N3dDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for vid in vids:
            ...
            # flow_imgs = flow_to_image(flow).numpy().transpose(1,2,0)

            # print(flow_imgs.max(), flow_imgs.min(),  flow_imgs.shape)
            # kiui.write_image("t.png",flow_imgs)
            # # img = F.to_pil_image(flow_imgs)
            # # img.save("test.png")
            # exit()
        ...
        stream_path = os.path.join(os.path.join(self.cfg.root_dir, scene_name, cur_frame), self.cfg.gs_mode,"stream",f"iteration_{self.cfg.iter}", "stream_500",next_frame)
        ...
        del rays, bbox, translate, radius, c2ws, c2ws_input, cur_images, next_images, depth_images, cam_centers, scene_info
        gc.collect()
    ...
    def collate(self, batch):
        ...
        # print(batch)
        if  self.training and self.cfg.use_gstream:
            stream_masks = []
            stream_dxyzs = []
            stream_drots = []
            ...
            # print(stream_masks, stream_dxyzs, stream_drots)
            ...
            del stream_masks, stream_dxyzs, stream_drots
            # print("upade")
        # print(batch["gs"], batch["points"])
