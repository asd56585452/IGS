...
def readColmapSceneInfo(path, images, eval, llffhold=8, test_view =None):
    try:
    ...
    if eval:
        if test_view !=None:
            train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx !=test_view ]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx ==test_view  ] 
        else:       
