...
from diff_gaussian_rasterization_compress import GaussianRasterizationSettings as GaussianRasterizationSettings_compress, GaussianRasterizer as GaussianRasterizer_compress

from scene.gaussian_model import GaussianModel
# from scene.anchor_model import AnchorModel
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size, scaling_modifier = 1.0, require_coord : bool = True, require_depth : bool = True):
    ...
    # torch.save(means2D, "means2D.pt")
    # torch.save(shs, "shs.pt")
    ...
    # torch.save(cov3D_precomp, "cov3D_precomp.pt")
...
def count_render(
    ...
    raster_settings = GaussianRasterizationSettings_compress(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    rasterizer = GaussianRasterizer_compress(raster_settings=raster_settings)
    means3D = pc.get_xyz
    means2D = screenspace_points
    scales, opacity = pc.get_scaling_n_opacity_with_3D_filter
...
def render_anchor(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size, scaling_modifier = 1.0, require_coord : bool = True, require_depth : bool = True):
    """
    Render the scene. 
