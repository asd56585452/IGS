...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    # testing_iterations += [ i for i in range(4500,8500,100)]
    # saving_iterations += [ i for i in range(4500,8500,100)]
    first_iter = 0
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if dataset.loader == "N3D":
                        zmask = gaussians._xyz[:,2] < 4.5  
                        gaussians.prune_points(zmask) 
                        torch.cuda.empty_cache()
                    elif dataset.loader == "enerf":
                        zmask = gaussians._xyz[:,2] >20  
                        gaussians.prune_points(zmask) 
                        torch.cuda.empty_cache()
                        # zmask = gaussians._xyz[:,2] <6
                        # gaussians.prune_points(zmask) 
                        # torch.cuda.empty_cache()
                    elif dataset.loader == "meet":
                        zmask = gaussians._xyz[:, 2] < 4.5

                        # # 构建 bbox_min 和 bbox_max
                        # bbox_min = torch.tensor([-15, -30, 10], device="cuda")
                        # bbox_max = torch.tensor([-1, 6, 20], device="cuda")

                        # # 构建 mask1
                        # mask1 = (gaussians._xyz >= bbox_min) & (gaussians._xyz <= bbox_max)
                        # mask1 = mask1.all(dim=1).squeeze()

                        # bbox_min = torch.tensor([-15, -30, 10], device="cuda")
                        # bbox_max = torch.tensor([15, 0, 20], device="cuda")
                        # # 构建 mask2
                        # mask2 = (gaussians._xyz >= bbox_min) & (gaussians._xyz <= bbox_max)
                        # mask2 = mask2.all(dim=1).squeeze()

                        # # 对 mask1 和 mask2 进行逐元素的逻辑或操作
                        # combined_mask = torch.logical_or(mask1, mask2)
                        # print(zmask.shape, combined_mask.shape)
                        # # 对 zmask 和 combined_mask 进行逐元素的逻辑或操作
                        # final_mask = torch.logical_or(zmask, combined_mask)
...
def training_report(tb_writer, iteration, Ll1, loss, normal_loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        print("pointnum", scene.gaussians.get_xyz.shape)
        torch.cuda.empty_cache()
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    #     os.makedirs(f"/workspace/{iteration}", exist_ok=True)
                    #     torchvision.utils.save_image(image, f"/workspace/{iteration}/img_{idx}.png")
                    #     print("save train")
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--test_iterations", nargs="+", type=int, default=[4000,7_000, 10_000, 15_000,30_000])
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=[ 4000, 7_000, 10_000, 15_000,30_000])
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[4000,4500,4600,4700,4800,4900,5000,5200,5400, 5500,5600,5700,5800,5900, 6000, 6500, 7_000, 10_000, 15_000,30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[4000,4500,4600,4700,4800,4900,5000, 5200,5400, 5500,5600,5700,5800,5900, 6000, 6500, 7_000, 10_000, 15_000,30_000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[15000])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    args = parser.parse_args(sys.argv[1:])#这个东西有模糊匹配的功能，即他是构建的一个字典树，能匹配上就是了 如 :iterat 可以匹配到iterations

    args.save_iterations.append(args.iterations)
    
    print("Optimizing " + args.model_path)
    print("this is rade-gs")
    # Initialize system state (RNG)
    safe_state(args.quiet)

    # args.test_iterations += [i for i in range(4000,20000, 1000)]
    # args.save_iterations += [i for i in range(4000,20000, 1000)]
